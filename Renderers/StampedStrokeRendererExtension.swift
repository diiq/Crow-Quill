...
extension Renderer {
  func stampedLine(start: Point, end: Point, stamper: (_ point: Point, _ renderer: Renderer) -> (), minimumGap: Double) {
    guard (end - start).length() > minimumGap else { return }
    let midPoint = (start + ((end - start) / 2)).withWeight((start.weight + end.weight) / 2)
    stamper(midPoint, self)
  ...
  func stampedLinear(_ points: [Point], stamper: (_ point: Point, _ renderer: Renderer) -> (), minimumGap: Double) {
    var lastPoint = points[0]
    points[1..<points.count].forEach {
      stamper(lastPoint, self)
  ...
  func bezierPoint(_ points: BezierPoints, t: Double) -> Point {
